...
import { GetUpcoming } from '@/redux/UpcomingSlice';
...
const Upcoming: React.FC = () => {
  ...
  const { movies, isLoading, error } = useSelector((state: RootState) => state.upcoming);
  const [randomMovies, setRandomMovies] = useState<Movie[]>([]);
  const [isPending, startTransition] = useTransition(); // Added useTransition hook
  ...
  if (isLoading && randomMovies.length === 0) { // Modified loading condition
  ...
  return (
    <><h1 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white ">Upcoming Movies</h1>
