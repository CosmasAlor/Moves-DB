      ...
      <div className="relative h-64 sm:h-72 md:h-96 lg:h-[28rem] xl:h-[32rem]  overflow-hidden">
        {randomMovies.map((movie, index) => (
          ...
          >
            ...
            <div className="absolute bottom-40 left-0 right-0 p-4  text-white">
              <h2 className="text-6xl font-bold">{movie.title}</h2>
              <p className="mt-2 text-2xl">{movie.overview.slice(0, 100)}...</p>
              <Link href={`/movie/${movie.id}`} className="mt-2 inline-block px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors duration-300">
                Read more
              </Link>
            </div>
          </div>
