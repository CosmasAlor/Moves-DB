...
const NowPlayingMovies: React.FC = () => {
  ...
  useEffect(() => {
    if (nowPlayingMovies.length > 0) {
      startTransition(() => {
        const filteredMovies = applyFilters(nowPlayingMovies);
        setDisplayedMovies(filteredMovies.slice(0, moviesPerPage));
      });
    }
  }, [nowPlayingMovies, filters]);
  ...
  const applyFilters = (movies: Movie[]) => {
    return movies.filter(movie => {
      ...
      return (
        ...
        releaseDate >= minDate &&
        releaseDate <= maxDate &&
        (filters.with_genres.length === 0 || movie.genre_ids.some(id => filters.with_genres.includes(id)))
      );
    }).sort((a, b) => {
  ...
  const loadMore = () => {
    ...
    const filteredMovies = applyFilters(nowPlayingMovies);
    const newDisplayedMovies = filteredMovies.slice(0, (currentPage + 1) * moviesPerPage);
  ...
  return (
    <div className="flex flex-col md:flex-row">
      <div className="w-full md:w-1/4 p-4 bg-gray-100 dark:bg-gray-800">
        <h1 className="mb-4 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Now Playing Movies</h1>
      ...
      <div className="w-full md:w-3/4">
        ...
        {nowPlayingMovies.length > displayedMovies.length && (
